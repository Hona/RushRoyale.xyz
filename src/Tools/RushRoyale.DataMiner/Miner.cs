...
public class Miner
{
    public static async Task MineAsync(bool headless = false)
    {
        AnsiConsole.Write(
        ...
        var defaultRushRoyaleDirectory = @"C:\GameCenter\Rush Royale PC";
        string rushRoyaleDirectory;

        if (headless)
        {
            rushRoyaleDirectory = defaultRushRoyaleDirectory;
        }
        else
        {
            rushRoyaleDirectory = AnsiConsole.Prompt(
                new TextPrompt<string>("Input the directory to Rush Royale's installation?")
                    .DefaultValue(defaultRushRoyaleDirectory));
        }
        ...
        foreach (var kvp in parts.Chunk(2))
        {
            ...
            var key = kvp[0].Trim().Trim(trimChars);
            var value = kvp[1].Trim().Trim(trimChars);
